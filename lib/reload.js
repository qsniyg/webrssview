...
var useragent = "FeedFetcher-Google; (+http://www.google.com/feedfetcher.html)";
...
function process_items(url, db_items, items) {
  ...
  for (var item of items) {
    ...
    if (item.pubDate)
      content.created_at = item.pubDate.getTime();
    if (item.date)
      content.updated_at = item.date.getTime();

    /*if (!content.created_at) {
      if (content.updated_at)
        content.created_at = content.updated_at;
      else
        content.created_at = content.added_at;
    }*/
    ...
    if (db_item) {
      ...
      if (check_updated_item(db_item, content)) {
        ...
        promises.push(db.update_content(db_item._id, {
          "$set": content
        }));
      }
    } else {
...
function reload_inner(url) {
  return new Promise((resolve, reject) => {
    ...
    var get_timing_str = function() {
      var now = Date.now();
      return "(" + (now - start) + ", " + (read_end - start) + ")";
    };

    var do_resolve = function() {
      //console.log(url + " " + get_timing_str());
      changed = feed_utils.set_feeds(url_feeds, {"error": null}) || changed;
      return resolve(changed);
    };

    var do_reject = function(message) {
      //console.log(message + " " + get_timing_str());
      console.log(message);
      ...
      (data) => {
        ...
        db.find_content({
          ...
          process_items(url, db_items, data.items).then(
            (retval) => {
              if (retval.inserts.length > 0) {
                ...
                db.insert_content(retval.inserts).then(
                  ...
                  () => {
                    do_reject("Error adding new content to mongo");
            ...
            (err) => {
              console.error(err);
              do_reject("Error processing items");
...
function reload_feed_promise(url, options, resolve, reject) {
  if (!options) {
    options = {};
  }
  ...
  if (url in reload_feed_list[options.thread].urls) {
    ...
    if (resolve)
      obj.resolve.push(resolve);
    if (reject)
      obj.reject.push(reject);
...
function reload_feed(url, options) {
  return new Promise((resolve, reject) => {
    reload_feed_promise(url, options, resolve, reject);
  });
}
module.exports.reload_feed = reload_feed;

function reload_feed_nopromise(url, options) {
  return reload_feed_promise(url, options);
}
module.exports.reload_feed_nopromise = reload_feed_nopromise;
